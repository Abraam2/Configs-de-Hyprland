...
function M.run_code()
  ...
  if not target_buf then
    vim.notify("󱐌 No se puede ejecutar. Revisa el foco.", vim.log.levels.WARN, { title = "Runner" })
    return
  end
  ...
  if vim.fn.executable(config.exe) == 0 then
    vim.notify(
      "󰚌 El programa '" .. config.exe .. "' no está instalado.",
  ...
  elseif ft == "java" then
    ...
    final_exec_cmd = config.cmd .. vim.fn.shellescape(file)
  end
