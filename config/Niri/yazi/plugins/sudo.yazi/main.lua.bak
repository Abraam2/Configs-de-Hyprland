...
-- --- FUNCIÓN DE EJECUCIÓN CORREGIDA Y SEGURA ---
local function execute(command)
	-- 1. Pedimos la contraseña ocultando los caracteres (obscure = true)
	local password, event = ya.input({
		title = "Contraseña de Root (Sudo):",
		pos = { "center", y = 0, w = 50 },
		obscure = true, -- <-- Oculta el texto plano con asteriscos/puntos
	})

	if event ~= 1 or password == "" then
		return
	end

	-- 2. Inyectamos la contraseña de forma silenciosa
	local final_cmd = { "echo", ya.quote(password), "|", "sudo", "-S" }

	for i = 2, #command do
		table.insert(final_cmd, command[i])
	end

	-- 3. Ejecutamos de forma asíncrona (block = false) para fulminar el parpadeo
	ya.emit("shell", {
		table.concat(final_cmd, " "),
		block = false, -- <-- Adiós parpadeo negro de pantalla
		confirm = false,
	})
end
